...
async function unlinkIfPresent(file, fsPromises) {
  try {
...
async function atomicWriteJson(file, payload, fsPromises = fs.promises) {
  ...
  try {
    ...
    try {
      await unlinkIfPresent(temp, fsPromises);
    } catch (cleanupError) {
...
function createRoomStore({
  ...
  async function saveSnapshot() {
    ...
    await Promise.all(current.map((room) =>
      atomicWriteJson(
        path.join(roomsDir, `${room.code}.json`),
        envelope(room),
        fsPromises
      )));
    const names = await fsPromises.readdir(roomsDir);
    await Promise.all(names
      .filter((name) => name.endsWith(".json") && !validCodes.has(name.slice(0, -5)))
      .map((name) => fsPromises.unlink(path.join(roomsDir, name))));
  ...
  async function backupSnapshot(label = "manual") {
    ...
    const stamp = new Date().toISOString().replace(/[:.]/g, "-");
    await atomicWriteJson(
      path.join(dir, `${stamp}_rooms_${label}.json`),
      { schema, productVersion, savedAt: Date.now(), rooms: current },
      fsPromises
    );
    const files = (await fsPromises.readdir(dir))
      .filter((name) =>
        name.includes("_rooms_") && name.endsWith(".json"))
      .sort()
      .reverse();
    await Promise.all(
      files
        .slice(backupLimit)
        .map((name) => fsPromises.unlink(path.join(dir, name)))
    );
    return "completed";
  }
