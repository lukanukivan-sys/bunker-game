...
const SAFE_MESSAGES = Object.freeze({
  ...
  PROBE_WRITE_FAILED: "Storage probe could not write its control file.",
  PROBE_READ_FAILED: "Storage probe could not read its control file.",
  PROBE_CONTENT_MISMATCH: "Storage probe control content did not match.",
  PROBE_DELETE_FAILED: "Storage probe control file could not be removed.",
...
function probeError(code, cause = null) {
  const error = new Error(SAFE_MESSAGES[code] || SAFE_MESSAGES.STORAGE_PROBE_FAILED);
  error.code = code;
  if (cause) error.cause = cause;
  return error;
}

async function awaitAllStartupBackups(backupTasks) {
  const results = await Promise.allSettled(
    backupTasks.map((backup) => Promise.resolve().then(backup))
  );
  ...
  if (failures.length) {
    const error = new AggregateError(
      failures,
      "One or more startup backups failed."
    );
...
function createPersistenceStatus({
  ...
  async function runWriteReadDeleteProbe() {
    ...
    try {
      try {
        await probeFs.mkdir(resolvedDataDir, { recursive: true });
        await probeFs.writeFile(probeFile, expected, {
          encoding: "utf8",
          flag: "wx"
        });
      } catch (error) {
        throw probeError("PROBE_WRITE_FAILED", error);
      }

      let actual;
      try {
        actual = await probeFs.readFile(probeFile, "utf8");
      } catch (error) {
        throw probeError("PROBE_READ_FAILED", error);
      }
      if (actual !== expected) {
        throw probeError("PROBE_CONTENT_MISMATCH");
      }

      try {
        await probeFs.unlink(probeFile);
      } catch (error) {
        throw probeError("PROBE_DELETE_FAILED", error);
      }
      try {
        await probeFs.access(probeFile);
        throw probeError("PROBE_DELETE_FAILED");
      } catch (error) {
        if (error.code !== "ENOENT") {
          if (error.code === "PROBE_DELETE_FAILED") throw error;
          throw probeError("PROBE_DELETE_FAILED", error);
        }
      }
      ...
      try {
        ...
        if (cleanupError.code !== "ENOENT") {
          failure = probeError("PROBE_DELETE_FAILED", cleanupError);
        }
      }
      try {
        await probeFs.access(probeFile);
        failure = probeError("PROBE_DELETE_FAILED");
      } catch (cleanupCheckError) {
        if (cleanupCheckError.code !== "ENOENT") {
          failure = cleanupCheckError.code === "PROBE_DELETE_FAILED"
            ? cleanupCheckError
            : probeError("PROBE_DELETE_FAILED", cleanupCheckError);
        }
      }
      const failureCode = SAFE_MESSAGES[failure.code]
        ? failure.code
      ...
      state.startupProbe = {
        status: "failed",
        completedAt: iso(),
        error: safeError(failureCode)
      };
      setStartupError("STORAGE_PROBE_FAILED", failure);
      return false;
    }
  }
