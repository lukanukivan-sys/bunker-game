...
const registered = platform.register({ username: "tester_one", displayName: "Тестер", password: "secret12" });
assert(registered.accountId && registered.token);
assert.throws(() => platform.register({ username: "tester_one", displayName: "Дубль", password: "secret12" }), /існує/);
const logged = platform.login({ username: "tester_one", password: "secret12" });
...
const relogged = reloaded.login({ username: "tester_one", password: "secret12" });
...
console.log("Платформні системи 1.0.5 перевірено: профілі, кампанії, набори, статистика й повторне завантаження.");
